...
//	          	List<Double> RelvanceRank = new ArrayList<>();
				 ...
				 for (String word : FinalQuery) {
					 ...
					 while(rs.next()){
						 ...
						 query = "SELECT * FROM rankedurls1 WHERE Urls = '"+tempUrl+"' AND id= '"+id+"' AND searchQuery= '"+SearchQuery+"' AND image='"+0+"'";
				 ...
				 {   query="SELECT * FROM rankedurls1 WHERE id='"+id+"' AND image= '"+0+"' AND searchQuery= '"+SearchQuery+"' ORDER BY Rank DESC ";
					 rs =  st.executeQuery(query);
					 while(rs.next())
					 {
						 tempUrl=rs.getString("Urls");
						 ...
						 while (matcher.find()) {
							 ...
							 UpdateQuery="UPDATE `rankedurls1` SET `Rank`= '"+TotalRank +"' WHERE Urls = '"+tempUrl+"' AND id= '"+id+"' AND image= '"+0+"' AND searchQuery= '"+SearchQuery+"'";
							 st_InsertFinalRank.executeUpdate(UpdateQuery);
